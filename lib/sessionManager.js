...
export default class SessionManager extends EventEmitter {
  ...
  _isPermanentDisconnect(lastDisconnect) {
    ...
    // prefer well-structured shapes if provided by libraries like Baileys
    const statusCode =
    ...
    if (typeof statusCode === "number") {
      ...
      // treat some special codes as transient (e.g. 515) by NOT returning true
    }
  ...
  async _handleConnectionUpdate(sessionId, update) {
    ...
    if (connection === "open") {
      ...
      entry.backoffMs = this.defaultBackoff;
      entry.restarting = false;
      // clear any scheduled reconnect (socket is open)
    ...
    if (connection === "close") {
      ...
      // If transient: schedule reconnect with exponential backoff
      if (!entry.restarting) {
        ...
        const timer = setTimeout(async () => {
          try {
            ...
            // double backoff for next time but clamp
            const curEntry = this.sessions.get(sessionId);
            if (!curEntry) return;
            curEntry.restarting = false;
            curEntry.backoffMs = Math.min(
              (curEntry.backoffMs || this.defaultBackoff) * 2,
              this.maxBackoff
            );
        ...
        // update stored entry (so other operations can see the timer/backoff state)
        this.sessions.set(sessionId, entry);
      }
    }
  }
